...
uses
    DAEffect, DAEffectX, DAudioEffect;
...
type
    ...
    public
      ...
      function String2parameter(Index: VstInt32; Text: pchar): boolean; virtual;                          // Convert a string representation to a parameter value
      ...
      function GetProgramNameIndexed(Category, Index: VstInt32; Text: pchar): boolean; virtual; // Fill text with name of program index (category deprecated in VST 2.4)
      ...
      // Host Properties
      function GetHostVendorString(Text: pchar): boolean; virtual;                                                           // Fills text with a string identifying the vendor
      function GetHostProductString(Text: pchar): boolean; virtual;                                                          // Fills text with a string with product name
      ...
      function CanHostDo(Text: pchar): VstInt32; virtual;                                                                    // Reports what the Host is able to do (hostCanDos in audioeffectx.cpp)
      ...
      function GetEffectName(Name: pchar): boolean; virtual;                                                           // Fill text with a string identifying the effect
      function GetVendorString(Text: pchar): boolean; virtual;                                                         // Fill text with a string identifying the vendor
      function GetProductString(Text: pchar): boolean; virtual;                                                        // Fill text with a string identifying the product name
      ...
      function CanDo(Text: pchar): VstInt32; virtual;                                                                  // Reports what the plug-in is able to do (plugCanDos in audioeffectx.cpp)
      ...
      {$IFDEF VST_2_3_EXTENSIONS}
      function GetNextShellPlugin(Name: pchar): VstInt32; virtual;        // This opcode is only called, if the plug-in is of type kPlugCategShell, in order to extract all included sub-plugin´s names.
      {$ENDIF} // VST_2_3_EXTENSIONS
      ...
      function GetErrorText(Text: pchar): boolean; virtual;
      ...
      function KeysRequired: boolean; virtual;
        ...
        {$IFDEF VST_2_2_EXTENSIONS}
      function GetChunkFile(NativePath: pchar): boolean; virtual;      // Returns in platform format the path of the current chunk (could be called in #setChunk ()) (FSSpec on MAC else char*)
        {$ENDIF} // VST_2_2_EXTENSIONS
...
function AudioEffectX.CanHostDo(Text: pchar): VstInt32;
begin
  Result := 0;
...
function AudioEffectX.GetProgramNameIndexed(Category, Index: VstInt32; Text: pchar): boolean;
begin
  Result := FALSE;
end;
...
function AudioEffectX.String2parameter(Index: VstInt32; Text: pchar): boolean;
begin
  Result := FALSE;
end;
...
{$IFDEF VST_2_3_EXTENSIONS}
function AudioEffectX.GetNextShellPlugin(Name: pchar): VstInt32;
begin
  StrCopy(Name, '');
  Result := 0;
end;
...
function AudioEffectX.GetChunkFile(NativePath: pchar): boolean;
begin
  Result := FALSE;
