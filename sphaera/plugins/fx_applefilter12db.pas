...
*)
  appendParameter( SParamText.create( 'processing',    0,   2,  txt_proc) );
  appendParameter( SParamFloat.create('cutoff (scale)',100, 0,  100) );
  appendParameter( SParamFloat.create('res (db)',      0,  -25, 25) );
  appendParameter( SParamFloat.create('output (db)',   0,  -25, 25) );
...
begin
  ...
  //coeffcients
  cutoff := 2 * cx / _sampleRate;
  res := power(10, 0.05 * -res);
  k := 0.5 * res * sin(S_PI * cutoff);
  c1 := 0.5 * (1 - k) / (1 + k);
  c2 := (0.5 + c1) * cos(S_PI * cutoff);
  c3 := (0.5 + c1 - c2) * 0.25;
