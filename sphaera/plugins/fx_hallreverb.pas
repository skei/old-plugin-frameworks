...
begin
  ...
  FVersion  := 0;
  FUniqueId := S_MAGIC + $0104;
  SSetFlag( FFlags,spf_perSample);
  appendParameter( SParamFloat.create('Dry',       0,  -48,  0   ) );
  appendParameter( SParamFloat.create('Wet',      -12, -48,  0   ) );
  appendParameter( SParamFloat.create('Length',    1,   0.01, 3   ) );
  appendParameter( SParamFloat.create('PreDelay',  10,  0,    100 ) );
  //
  tmp1 := 0;
  tmp2 := 0;
...
begin
  ...
  value := AValue;//param.from01(AValue);
...
begin
  if needrecalc then
  begin
    l0   := trunc( slider4/1000*_sampleRate+1 );
    l1   := 3;
    l2   := 13;
    l3   := 31;
    l4   := 103;
    ...
    l11  := 1213;
    l12  := 1553;
    Rl0  := trunc( slider4/1000*_sampleRate+1 );
    Rl1  := 3;
    Rl2  := 11;
    Rl3  := 37;
    Rl4  := 113;
    ...
    g9  := 0.62;
    g11 := 0.62;
    f   := Exp( Ln(0.001) / ( slider3*_sampleRate / (l5+l6+l7+l8) ) );
    d   := Exp( -2*S_PI*18000/_sampleRate );
