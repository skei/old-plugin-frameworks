...
constructor SDspRBJ.create{(SampleRate:single;MaxBlockSize:integer)};
begin
...
procedure SDspRBJ.SetQ(NewQ:single);
begin
  fQ := (1-NewQ) * 0.98;
end;
...
procedure SDspRBJ.CalcFilterCoeffs(aSampleRate:Single;pFilterType:integer;pFreq,pQ,pDBGain:single;pQIsBandWidth:boolean);
begin
...
procedure SDspRBJ.CalcFilterCoeffs(aSampleRate:Single);
var
...
function SDspRBJ.Process(input:single):single;
var
  LastOut : single;
begin
  // filter
