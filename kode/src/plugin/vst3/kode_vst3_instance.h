...
//------------------------------
  ...
  IComponentHandler*  MComponentHandler   = KODE_NULL;
  IComponentHandler2* MComponentHandler2  = KODE_NULL;
  IPlugFrame*         MPlugFrame          = KODE_NULL;
  IHostApplication*   MHostApp            = KODE_NULL;
  ParameterInfo*      MParamInfos         = KODE_NULL;
  IRunLoop*           MRunLoop            = KODE_NULL;
  uint32_t            MIoMode             = 0;
...
//------------------------------
  ...
  uint32_t getPluginFormat() override {
    return KODE_PLUGIN_FORMAT_VST3;
  }
...
//    KODE_BaseInstance::updateParameterFromEditor(AIndex,AValue);
//  }

  //----------

  //void resizeWindowFromEditor(uint32_t AWidth, uint32_t AHeight) override {
  //  KODE_BaseInstance::resizeWindowFromEditor(AWidth,AHeight) ;
  //}
  ...
  */
  ...
  void setupParameterInfo() {
    if (!MParamInfos) {
      ...
      MParamInfos = (ParameterInfo*)KODE_Malloc( num * sizeof(ParameterInfo) );
            ...
            if (id < MDescriptor->getNumParameters()) {
              ...
              switch(midi_ev) {
                case KODE_VST3_PARAM_AFTERTOUCH: {
                  //if (offset != 0)
                  MInstance->on_midiInput(offset,KODE_MIDI_CHANNEL_AFTERTOUCH+midi_ch,value*127.0f,0);
                  break;
                } // at
                case KODE_VST3_PARAM_PITCHBEND: {
                  ...
                  //if (midi_ch != 0) {
                    ...
                    uint32_t m3 = i2 >> 7;
                    //if (offset != 0)
                    MInstance->on_midiInput(offset,KODE_MIDI_PITCHBEND+midi_ch,m2,m3);
                  //}
                  break;
                } // pb
                case KODE_VST3_PARAM_BRIGHTNESS: {
                  //if (offset != 0)
                  MInstance->on_midiInput(offset,KODE_MIDI_CONTROL_CHANGE+midi_ch,74,value*127.0f);
                  break;
                }
              } // switch (midi_ev)
            } // id < num params
      ...
      for (int32_t i=0; i<num; i++) {
        ...
        switch (event.type) {
          case Event::kNoteOnEvent:
            ...
            MInstance->on_midiInput(offset,msg1,msg2,msg3);
...
//----------------------------------------
  ...
  tresult PLUGIN_API setState(IBStream* state) override {
    ...
    switch (mode) {
      case 0:
        ...
        MInstance->on_restoreState(size,ptr,0);
        KODE_Free(ptr);
        break;
      case 1:
  ...
  */
  ...
  tresult PLUGIN_API getState(IBStream* state) override {
    ...
    uint32_t  mode    = 0;
    void*     ptr     = KODE_NULL;
    uint32_t  size    = MInstance->on_saveState(&ptr,0);
    if ((size == 0) && (ptr == KODE_NULL)) {
      ptr = MInstance->getParamValues();
  ...
  */
  ...
  tresult PLUGIN_API getParameterInfo(int32 paramIndex, ParameterInfo& info) override {
    if (paramIndex >= 0) {
      if (paramIndex < (int32)MDescriptor->getNumParameters()) {
        ...
        if (param) {
          KODE_Memcpy(&info,&MParamInfos[paramIndex],sizeof(ParameterInfo));
          return kResultOk;
        }
      } // index < numparams
      else {
  ...
  ParamValue PLUGIN_API getParamNormalized(ParamID id) override {
    if (id < MDescriptor->getNumParameters()) {
      float v = MInstance->getParamValue(id);
      return v;
    }
    else {
      return 0;
  ...
  */
  ...
  tresult PLUGIN_API setParamNormalized(ParamID id, ParamValue value) override {
    ...
    MInstance->setParamValue(id,value);
  ...
  */
  ...
  IPlugView* PLUGIN_API createView(FIDString name) override {
    if (MDescriptor->hasFlag(KODE_PLUGIN_HAS_EDITOR)) {
      if (name && (KODE_Strcmp(name,ViewType::kEditor) == 0)) {
        addRef();
  ...
  tresult PLUGIN_API isPlatformTypeSupported(FIDString type) override {
    // "X11EmbedWindowID"
    if (type && KODE_Strcmp(type,kPlatformTypeX11EmbedWindowID) == 0) {
      return kResultOk;
    }
    return kResultFalse;
  }
  ...
  */
  ...
  tresult PLUGIN_API attached(void* parent, FIDString type) override {
      ...
      if (MDescriptor->hasFlag(KODE_PLUGIN_HAS_EDITOR)) {
        ...
        MEditor = MInstance->openEditor(parent);
        MEditor->open();
        //if (MRunLoop)
  ...
  */
  ...
  tresult PLUGIN_API getSize(ViewRect* size) override {
    if (MDescriptor->hasFlag(KODE_PLUGIN_HAS_EDITOR)) {
      size->left    = 0;
      size->top     = 0;
      size->right   = MDescriptor->getEditorDefaultWidth();
      size->bottom  = MDescriptor->getEditorDefaultHeight();
      return kResultOk;
    }
    return kResultFalse;
  }
