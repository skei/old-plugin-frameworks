...
xcb_atom_t
kode_xcb_get_intern_atom(xcb_connection_t *conn, const char *name) {
...
xcb_cursor_t
kode_xcb_create_font_cursor(xcb_connection_t *conn, uint16_t glyph) {
...
xcb_visualtype_t*
kode_xcb_find_visual(xcb_connection_t* c, xcb_visualid_t visual) {
...
void
kode_xcb_add_string_text8(KODE_XcbPolyText8* pt, char const* s) {
...
void
kode_xcb_add_delta_text8(KODE_XcbPolyText8* pt, uint8_t delta) {
  uint8_t* buffer;
...
void
kode_xcb_add_font_text8(KODE_XcbPolyText8* pt, xcb_font_t font) {
  uint8_t* buffer;
