...
: public KODE_Widget {
  ...
  public:
    ...
    : KODE_Widget(ARect) {
      ...
      wdg_src = KODE_New KODE_SelectorWidget( KODE_Rect(x,y,w1,h), ASrcMenu);
        wdg_src->setSelected(ASrc);
        wdg_src->setTextAlignment(KODE_TEXT_ALIGN_LEFT);
      appendWidget(wdg_src);
      x += w1 + 1;

      wdg_dst = KODE_New KODE_SelectorWidget( KODE_Rect(x,y,w1,h), ADstMenu);
        wdg_dst->setSelected(ADst);
        wdg_dst->setTextAlignment(KODE_TEXT_ALIGN_LEFT);
      appendWidget(wdg_dst);
      x += w1 + 1;

      wdg_amount = KODE_New KODE_DragValueWidget( KODE_Rect(x,y,w2,h), AAmt);
      ...
      wdg_scale = KODE_New KODE_DragValueWidget( KODE_Rect(x,y,w2,h), AScale);
      ...
      wdg_offset = KODE_New KODE_DragValueWidget( KODE_Rect(x,y,w2,h), AOffset);
      ...
      //wdg_rate = KODE_New KODE_SelectorWidget( KODE_Rect(x,y,w1,h), ARateMenu);
      //  wdg_rate->setSelected(ASrc);
      //  wdg_rate->setTextAlignment(KODE_TEXT_ALIGN_LEFT);
      //appendWidget(wdg_rate);
      //x += w1 + 1;

      //wdg_mode = KODE_New KODE_SelectorWidget( KODE_Rect(x,y,w1,h), AModeMenu);
...
public:
  ...
  KODE_ModMatrixRow* appendRow(uint32_t src=0, uint32_t dst=0, float amt=0.0f, float scale=1.0f, float offset=0.0f, uint32_t rate=0, uint32_t mode=0 ) {
    float y = MNumRows * 17;
    KODE_ModMatrixRow* item = KODE_New KODE_ModMatrixRow(
