#ifndef kode_file_included
#define kode_file_included
//----------------------------------------------------------------------
...
#define KODE_FILE_READ_BINARY    (char*)"rb"
#define KODE_FILE_READ_TEXT      (char*)"r"
#define KODE_FILE_WRITE_BINARY   (char*)"wb"
#define KODE_FILE_WRITE_TEXT     (char*)"w"
#define KODE_FILE_APPEND_BINARY  (char*)"ab"
#define KODE_FILE_APPEND_TEXT    (char*)"w"
...
class KODE_File {
  ...
  public:
    ...
    */
    ...
    KODE_File(const char* AFilename, const char* AMode=KODE_FILE_READ_BINARY) {
    ...
    bool exists(const char* AName) {
      ...
      FILE* fp = fopen(AName,KODE_FILE_READ_TEXT);
      if (fp) {
        close();
        return true;
      }
    ...
    uint32_t length(void) {
      ...
      if (MHandle) {
        ...
        FILE* fp = fopen(MName,KODE_FILE_READ_TEXT);
        if (!fp) return 0;
        fseek(fp,0,SEEK_END);
        len = ftell(fp);
        //fseek(fp,0,SEEK_SET);
    ...
    virtual
    bool open(const char* AFilename, const char* AMode=KODE_FILE_READ_BINARY) {
      MName   = AFilename;
      MMode   = AMode;
...
#include <stdio.h>
#include "kode_basepath.h"
#include "kode_const.h"
#include "kode_memory.h"
#include "kode_string.h"
#include "kode_utils.h"
