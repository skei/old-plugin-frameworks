...
const
  ...
  freqHP    = 2243.2;
  freqLP    = 224.32;
  freqHP_p2 = -Z_PI2 * freqHP;
  freqLP_p2 = -Z_PI2 * freqLP;
...
begin
  ...
  appendParameter( ZParamFloat.create('Low Cont', 0,  0,  10, 0.1 ) );
  appendParameter( ZParamFloat.create('Process',  0,  0,  10, 0.1 ) );
  appendParameter( ZParamFloat.create('Output',   0, -30, 0,  0.1 ) );
end;
...
begin
  case AState of
    zps_resume:
      begin
        xHP  := exp(freqHP_p2/FSampleRate);
        a0HP := 1.0 - xHP;
        b1HP := -xHP;
        xLP  := exp(freqLP_p2/FSampleRate);
        a0LP := 1.0 - xLP;
        b1LP := -xLP;
      end;
  end;
