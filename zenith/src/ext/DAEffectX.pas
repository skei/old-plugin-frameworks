...
const
     ...
     audioMasterGetVendorString             = audioMasterX + 28; // [ptr] char buffer for vendor string, limited to kVstMaxVendorStrLen. See AudioEffectX.getHostVendorString
     audioMasterGetProductString            = audioMasterX + 29; // [ptr] char buffer for vendor string, limited to kVstMaxProductStrLen. See AudioEffectX.getHostProductString
     ...
     audioMasterGetDirectory                = audioMasterX + 37; // [return value] FSSpec on MacOS, else pchar. See AudioEffectX.getDirectory
     ...
     audioMasterGetChunkFile                = audioMasterX + 44; // (deprecated) deprecated in VST 2.4 [ptr] char[2048] or sizeof(FSSpec) [return value] 1 if supported. See AudioEffectX.getChunkFile
...
type
    ...
    VstParameterProperties = record
      ...
      vLabel                  : array[0..kVstMaxLabelLen-1] of char;        // parameter label
      ...
      shortLabel              : array[0..kVstMaxShortLabelLen-1] of char;   // short label, recommended: 6 + delimiter
      ...
      categoryLabel           : array[0..kVstMaxCategLabelLen-1] of char;   // category label, e.g. "Osc 1"

      future                  : array[0..15] of char;                       // reserved for future use
    end;
...
type
    ...
    VstPinProperties = record
      vLabel          : array[0..kVstMaxLabelLen-1] of char;       // pin name
      flags           : VstInt32;                                  // see VstPinPropertiesFlags
      arrangementType : VstInt32;                                  // see VstSpeakerArrangementType
      shortLabel      : array[0..kVstMaxShortLabelLen-1] of char;  // short name (recommended: 6 + delimiter)
...
type
    ...
    MidiProgramName = record
      ...
      name                : array[0..kVstMaxNameLen-1] of char;  // program name
...
type
    ...
    MidiProgramCategory = record
      ...
      name                : array[0..kVstMaxNameLen-1] of char;   // name
      parentCategoryIndex : VstInt32;                             // -1:no parent category
...
type
    ...
    MidiKeyName = record
      ...
      keyName          : array[0..kVstMaxNameLen-1] of char;  // key name, empty means regular key names
...
type
    ...
    VstSpeakerProperties = record
      ...
      name      : array[0..kVstMaxNameLen-1] of char;    // for new setups, new names should be given (L/R/C... won't do)
...
type
    ...
    VstAudioFile = record
      ...
      name                 : array[0..kVstMaxFileNameLen-1] of char;	// file title
...
type
    ...
    VstFileSelect = record
      ...
      returnPath          : pchar;                     // use with #kVstFileLoad and #kVstDirectorySelect. null: Host allocates memory, plug-in must call #closeOpenFileSelector!
      sizeReturnPath      : VstInt32;                  // size of allocated memory for return paths
      returnMultiplePaths : ^pchar;                    // use with kVstMultipleFilesLoad. Host allocates memory, plug-in must call #closeOpenFileSelector!
...
type
    ...
    VstPatchChunkInfo = record
      ...
      future         : array[0..47] of char;    // Reserved for future use
    end;
