...
procedure PitchShift(pitchShift: Single; numSampsToProcess, fftFrameSize,
  osamp: Longint; sampleRate: Single; indata, outdata: PSingle);
...
procedure smbFFT(fftBuffer: PSingle; fftFrameSize, sign: Longint); forward;
function smbAtan2(x, y: Double): Double; forward;
...
begin
  ...
  begin
    ...
    begin
      ...
      FillChar(gSynMagn, SizeOf(gSynMagn), 0);
      FillChar(gSynFreq, SizeOf(gSynFreq), 0);
      for k := 0 to fftFrameSize2 - 1 do
      begin
      ...
      begin
        window := -0.5 * Cos(2.0 * Pi * k/fftFrameSize) + 0.5;
        gOutputAccum[k] := gOutputAccum[k] +
                           2.0 * window * gFFTworksp[2 * k] /
                           (fftFrameSize2 * osamp);
      end;
      ...
      { shift accumulator }
      Move(gOutputAccum[stepSize], gOutputAccum[0],
        fftFrameSize * sizeof(Single));
