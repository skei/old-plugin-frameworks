...
type
  ...
  private
    ...
    P           : PMarkovNode;
    N           : PMarkovNode;
    nodes       : array[0..255,0..255] of ZMarkovNode;
    nodebuf     : PMarkovNode;
...
const
  z_markov_eof       = -1;
  z_markov_threshold = 8;
  z_markov_bigthresh = 8;

//----------------------------------------------------------------------

constructor ZMarkov.create;
begin
  inherited;
  rbuf := nil;
  rbufsize := 0;
  ...
  wbufpos := 0;
  memsize  := $1000000;
  threshold := z_markov_threshold;//2;
  bigthresh := z_markov_bigthresh;//2;
  nodebuf := nil;
end;

//----------

destructor ZMarkov.destroy;
begin
  inherited;
end;
...
function ZMarkov.get_char : LongInt;
var
  c : Byte;
begin
  //return getchar();
  ...
  else result := z_markov_eof;//-1;
end;

//----------

procedure ZMarkov.put_char(c:LongInt);
var
  ch : Byte;
begin
  //putchar(c);
...
procedure ZMarkov.init(msize:LongInt);
begin
...
function ZMarkov.predict : Single;
begin
...
procedure ZMarkov.update(b:LongInt);
var
  r : Single;
  //temp : Single;
begin
...
function ZMarkov.compress(outbuf,inbuf:PByte; insize:LongInt) : LongInt;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if bit <> 0 then update(Z_TRUE) else update(Z_FALSE); // p_update(bit != 0);
    ...
    //if (!(++inbytes & 0xff))
    inbytes += 1;
    if (inbytes and $ff) = Z_FALSE then
    begin
      //if (!(inbytes & 0xffff))
      //{
...
function ZMarkov.expand(outbuf,inbuf:PByte; insize:LongInt) : LongInt;
var
...
begin
  ...
  begin
    ...
    outbytes += 1;
    if (outbytes and $ff) = Z_FALSE then
    begin
