...
function ZStrcat(s1,s2:PChar) : PChar;
function ZStrchr(s:PChar; c:Char) : PChar;
function ZStrcmp(s1,s2:PChar) : longword;
function ZStrcpy(dst,src:PChar) : PChar;
function ZStrdup(src:PChar) : PChar;
function ZStrlen(s:PChar) : integer;
function ZStrncpy(dst,src:PChar; n:longword) : PChar;
function ZStrrchr(s:PChar; c:Char) : PChar;
...
function ZStrdup(src:PChar) : PChar;
begin
  result := GetMem(ZStrlen(src)+1);
  if assigned(result) then ZStrcpy(result,src);
end;
...
function ZStrncmp(s1,s2:PChar; n:longword) : longword;
var u1,u2:Byte;//Char;
begin
  while n>0 do
  begin
