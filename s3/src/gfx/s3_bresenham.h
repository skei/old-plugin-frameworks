...
#ifndef s3_bresenham_included
#define s3_bresenham_included
//----------------------------------------------------------------------
...
/*
  ...
  be careful...
  this is included directly into SBitmap (skei_bitmap.h)
*/

//----------------------------------------------------------------------

/*
void drawLine(int x0, int y0, int x1, int y1, SColor AColor);
void drawLineAA(int x0, int y0, int x1, int y1, SColor AColor);
void drawLineWidth(int x0, int y0, int x1, int y1, float wd, SColor AColor);

void drawEllipse(int xm, int ym, int a, int b, SColor AColor);
void drawOptimizedEllipse(int xm, int ym, int a, int b, SColor AColor);
void drawEllipseRect(int x0, int y0, int x1, int y1, SColor AColor);
void drawEllipseRectAA(int x0, int y0, int x1, int y1, SColor AColor);
void drawRotatedEllipse(int x, int y, int a, int b, float angle, SColor AColor);
void drawRotatedEllipseRect(int x0, int y0, int x1, int y1, long zd, SColor AColor);

void drawCircle(int xm, int ym, int r, SColor AColor);
void drawCircleAA(int xm, int ym, int r, SColor AColor);

void drawQuadBezierSeg(int x0, int y0, int x1, int y1, int x2, int y2, SColor AColor);
void drawQuadBezierSegAA(int x0, int y0, int x1, int y1, int x2, int y2, SColor AColor);
void drawQuadBezier(int x0, int y0, int x1, int y1, int x2, int y2, SColor AColor);

void drawQuadRationalBezier(int x0, int y0, int x1, int y1, int x2, int y2, float w, SColor AColor);
void drawQuadRationalBezierSeg(int x0, int y0, int x1, int y1, int x2, int y2, float w, SColor AColor);
void drawQuadRationalBezierSegAA(int x0, int y0, int x1, int y1, int x2, int y2, float w, SColor AColor);

void drawCubicBezier(int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3, SColor AColor);
void drawCubicBezierSeg(int x0, int y0, float x1, float y1, float x2, float y2, int x3, int y3, SColor AColor);
void drawCubicBezierSegAA(int x0, int y0, float x1, float y1, float x2, float y2, int x3, int y3, SColor AColor);

void drawQuadSpline(int n, int x[], int y[], SColor AColor);
void drawCubicSpline(int n, int x[], int y[], SColor AColor);
*/
...
{                                    /* draw an anti-aliased line of width wd */
   ...
   for (wd = (wd+1)/2; ; ) {                                    /* pixel loop */
      blendPixel/*Color*/(x0, y0, color, 255-SMaxI(0,255*(abs(err-dx+dy)/ed-wd+1)) );
      e2 = err; x2 = x0;
      if (2*e2 >= -dx) {                                            /* x step */
         for (e2 += dy, y2 = y0; e2 < ed*wd && (y1 != y2 || dx > dy); e2 += dx)
            blendPixel/*Color*/(x0, y2 += sy, color, 255-SMaxI(0,255*(abs(e2)/ed-wd+1)) );
         if (x0 == x1) break;
         e2 = err; err -= dy; x0 += sx;
      }
      if (2*e2 <= dy) {                                             /* y step */
         for (e2 = dx-e2; e2 < ed*wd && (x1 != x2 || dx < dy); e2 += dy)
            blendPixel/*Color*/(x2 += sx, y0, color, 255-SMaxI(0,255*(abs(e2)/ed-wd+1)) );
         if (y0 == y1) break;
         err += dx; y0 += sy;
      }
   }
...
void drawEllipse(int xm, int ym, int a, int b, SColor AColor)
{
   ...
   do {
       ...
       setPixel(xm+x, ym+y, color);                                 /*  II. Quadrant */
       setPixel(xm+x, ym-y, color);                                 /* III. Quadrant */
       setPixel(xm-x, ym-y, color);                                 /*  IV. Quadrant */
       e2 = 2*err;
   ...
   while (y++ < b) {                  /* too early stop of flat ellipses a=1, */
       setPixel(xm, ym+y, color);                        /* -> finish tip of ellipse */
...
{                                   /* draw ellipse rotated by angle (radian) */
  ...
  drawRotatedEllipseRect(x-a,y-b, x+a,y+b, (long)(4*zd*cos(angle)),AColor);
}
...
{                  /* rectangle enclosing the ellipse, integer rotation angle */
   ...
   float w = xd*(long)yd;
   if (zd == 0) return drawEllipseRect(x0,y0, x1,y1,AColor);          /* looks nicer */
   if (w != 0.0) w = (w-zd)/(w+w);                    /* squared weight of P1 */

   //assert(w <= 1.0 && w >= 0.0);                /* limit angle to |zd|<=xd*yd */
   SAssert( (w <= 1.0) && (w >= 0.0) );                /* limit angle to |zd|<=xd*yd */

   xd = floor(xd*w+0.5); yd = floor(yd*w+0.5);           /* snap xe,ye to int */
   drawQuadRationalBezierSeg(x0,y0+yd, x0,y0, x0+xd,y0, 1.0-w,AColor);
   drawQuadRationalBezierSeg(x0,y0+yd, x0,y1, x1-xd,y1, w,AColor);
   drawQuadRationalBezierSeg(x1,y1-yd, x1,y1, x1-xd,y1, 1.0-w,AColor);
   drawQuadRationalBezierSeg(x1,y1-yd, x1,y0, x0+xd,y0, w,AColor);
}
...
void drawCircle(int xm, int ym, int r, SColor AColor)
{
   ...
   do {
      setPixel(xm-x, ym+y, color);                            /*   I. Quadrant +x +y */
      setPixel(xm-y, ym-x, color);                            /*  II. Quadrant -x +y */
      setPixel(xm+x, ym-y, color);                            /* III. Quadrant -x -y */
      setPixel(xm+y, ym+x, color);                            /*  IV. Quadrant +x -y */
      r = err;
...
{                     /* draw a black anti-aliased circle on white background */
   ...
   do {
      ...
      blendPixel(xm-x, ym+y,color, 255-i);                             /*   I. Quadrant */
      blendPixel(xm-y, ym-x,color, 255-i);                             /*  II. Quadrant */
      blendPixel(xm+x, ym-y,color, 255-i);                             /* III. Quadrant */
      blendPixel(xm+y, ym+x,color, 255-i);                             /*  IV. Quadrant */
...
{                                  /* draw a limited quadratic Bezier segment */
  ...
  if (cur != 0) {                                         /* no straight line */
    ...
    do {
      setPixel(x0,y0, color);                                          /* draw curve */
  ...
  drawLine(x0,y0, x2,y2,AColor);                       /* draw remaining part to end */
}
...
{                    /* draw an limited anti-aliased quadratic Bezier segment */
   ...
   {                                                      /* no straight line */
      ...
      do {
         ...
         blendPixel(x0,y0, color,255-(255*fabs(err-dx-dy-xy)/ed));          /* draw curve */
   ...
   drawLineAA(x0,y0, x2,y2,AColor);                  /* draw remaining needle to end */
}
...
{                                          /* draw any quadratic Bezier curve */
   ...
   //drawQuadBezierSeg(x0,y0, x1,y1, x2,y2,AColor);                  /* remaining part */
   drawQuadBezierSegAA(x0,y0, x1,y1, x2,y2,AColor);                  /* remaining part */
}

//----------------------------------------------------------------------
// quad rational bezier
//----------------------------------------------------------------------

void drawQuadRationalBezierSeg(int x0, int y0, int x1, int y1,
                               int x2, int y2, float w, SColor AColor)
{                   /* draw a limited rational Bezier segment, squared weight */
  ...
  if (cur != 0.0 && w > 0.0) {                            /* no straight line */
    ...
    if (w < 0.5 && (dy > xy || dx < xy)) {   /* flat ellipse, algorithm fails */
       ...
       drawQuadRationalBezierSeg(x0,y0, dx,dy, sx,sy, cur,AColor);/* draw separately */
  ...
  drawLine(x0,y0, x2,y2,AColor);                     /* draw remaining needle to end */
}

//----------------------------------------------------------------------

void drawQuadRationalBezierSegAA(int x0, int y0, int x1, int y1,
                                 int x2, int y2, float w, SColor AColor)
{   /* draw an anti-aliased rational quadratic Bezier segment, squared weight */
   ...
   if (cur != 0.0 && w > 0.0) {                           /* no straight line */
      ...
      if (w < 0.5 && dy > dx) {              /* flat ellipse, algorithm fails */
         ...
         sy = floor((y0+2.0*w*y1+y2)*xy/2.0+0.5);
         dx = floor((w*x1+x0)*xy+0.5); dy = floor((y1*w+y0)*xy+0.5);
         drawQuadRationalBezierSegAA(x0,y0, dx,dy, sx,sy, cur,AColor); /* draw apart */
         dx = floor((w*x1+x2)*xy+0.5); dy = floor((y1*w+y2)*xy+0.5);
      ...
      do {                                                      /* pixel loop */
         ...
         if (x1 < 256) blendPixel(x0,y0, color,255-x1);                   /* draw curve */
...
void drawQuadRationalBezier(int x0, int y0, int x1, int y1,
                            int x2, int y2, float w, SColor AColor)
{                                 /* draw any quadratic rational Bezier curve */
   ...
   drawQuadRationalBezierSeg(x0,y0, x1,y1, x2,y2, w*w, AColor);          /* remaining */
}
...
void drawCubicBezierSeg(int x0, int y0, float x1, float y1,
                        float x2, float y2, int x3, int y3, SColor AColor)
...
exit: xx = x0; x0 = x3; x3 = xx; sx = -sx; xb = -xb;             /* swap legs */
   ...
   drawLine(x0,y0, x3,y3,AColor);       /* remaining part in case of cusp or crunode */
}

//----------------------------------------------------------------------

void drawCubicBezierSegAA(int x0, int y0, float x1, float y1,
                          float x2, float y2, int x3, int y3, SColor AColor)
{                           /* draw limited anti-aliased cubic Bezier segment */
   ...
   SAssert((x1-x0)*(x2-x3) < EP && ((x3-x0)*(x1-x2) < EP || xb*xb < xa*xc+EP));
   SAssert((y1-y0)*(y2-y3) < EP && ((y3-y0)*(y1-y2) < EP || yb*yb < ya*yc+EP));
   ...
   do {                                                /* loop over both ends */
      ...
      for (fx = fy = f; x0 != x3 && y0 != y3; ) {
         ...
         if (y1 < 256) blendPixel(x0, y0, color,255-y1);                  /* draw curve */
         ...
         if (2*fy <= f) {                           /* x+ anti-aliasing pixel */
            if (py < ed) blendPixel(x0+sx, y0, color, 255-(255*py/ed));      /* draw curve */
            y0 += sy; fy += f;                                      /* y step */
         }
         if (2*fx <= f) {                           /* y+ anti-aliasing pixel */
            if (px < ed) blendPixel(x0, y2+sy, color, 255-(255*px/ed));      /* draw curve */
            x0 += sx; fx += f;                                      /* x step */
         }
      }
...
exit:
   ...
   drawLineAA(x0,y0, x3,y3,AColor);     /* remaining part in case of cusp or crunode */
}

//----------------------------------------------------------------------

void drawCubicBezier(int x0, int y0, int x1, int y1,
                     int x2, int y2, int x3, int y3, SColor AColor)
...
{                         /* draw quadratic spline, destroys input arrays x,y */
   ...
   SAssert(n > 1);                        /* need at least 3 points P[0]..P[n] */
   ...
   drawQuadBezier(x[0],y[0], x1,y1, x2,y2, AColor);
   #undef M_MAX
}
...
{                             /* draw cubic spline, destroys input arrays x,y */
   ...
   SAssert(n > 2);                        /* need at least 4 points P[0]..P[n] */
   ...
   drawCubicBezier(x3,y3, (x2+x4)/2,(y2+y4)/2, x4,y4, x4,y4, AColor);
   ...
   drawCubicBezier(x4,y4, floor((2*x1+x2)/3+0.5),floor((2*y1+y2)/3+0.5),
                   ...
                   AColor);
   drawCubicBezier(x0,y0, x0,y0, (x0+x1)/2,(y0+y1)/2, x4,y4, AColor);
   #undef M_MAX
}
