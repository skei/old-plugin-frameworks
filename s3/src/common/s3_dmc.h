#ifndef s3_dmc_included
#define s3_dmc_included
//----------------------------------------------------------------------
/*
...
//#define S3_DMC_THRESHOLD 2 // 8
//#define S3_DMC_BIGTHRESH 2 // 8
...
class S3_DMC {
  ...
  private:
    ...
    s3_dmc_node   *P;
    s3_dmc_node   *N;
    s3_dmc_node   nodes[256][256];
    s3_dmc_node   *nodebuf;
    s3_dmc_node   *nodemaxp;
    s3_dmc_node   *nodesptr;
  ...
  public:
    ...
    S3_DMC() {
      ...
      wbufpos = 0;
      memsize   = 0x1000000;
      threshold = S3_DMC_THRESHOLD;//2;
      bigthresh = S3_DMC_BIGTHRESH;//2;
      nodebuf = S3_NULL;
    }
    ...
    void p_init(int memsize) {
      ...
      nodebuf = (s3_dmc_node *)S3_Malloc(memsize);
      //if (nodebuf == (s3_dmc_node *)NULL)
      //{
      //  fprintf(stderr,"memory alloc failed; try smaller predictor memory\n");
      //  exit(1);
      //}
      nodemaxp = nodebuf + (memsize/sizeof(s3_dmc_node)) - 20;
      p_flush();
    }
