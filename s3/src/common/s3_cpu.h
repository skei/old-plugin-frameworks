#ifndef s3_cpu_included
#define s3_cpu_included
//----------------------------------------------------------------------
...
// see also: S3_LITTLE_ENDIAN

bool S3_IsLittleEndian(void) {
...
class S3_FpuState {
  ...
  public:
    ...
    S3_FpuState(Rounding mode = kRoundToZero) {
...
// fPIC compatible
#define S3_CPU_EBX_REG "r"

#define S3_CPU_EBX_STORE  \
  "pushl %%ebx;"            \
  "cpuid;"                  \
  "movl %%ebx, %1;"         \
  "popl %%ebx;"
...
class SCPU {
  ...
  private:
    ...
    char          cpustringbuf[S3_MAX_STRING_SIZE];
    unsigned int  _caps;
