#ifndef s2_pitch_tracker_included
#define s2_pitch_tracker_included
//----------------------------------------------------------------------
...
class S2_PitchTracker {
  ...
  public:
    ...
    double _dywapitch_computeWaveletPitch(double * samples, int startsample, int samplecount) {
      ...
      double *sam = (double *)S2_Malloc(sizeof(double)*samplecount);
      memcpy(sam, samples + startsample, sizeof(double)*samplecount);
      int curSamNb = samplecount;
      int *distances = (int *)S2_Malloc(sizeof(int)*samplecount);
      int *mins = (int *)S2_Malloc(sizeof(int)*samplecount);
      int *maxs = (int *)S2_Malloc(sizeof(int)*samplecount);
      int nbMins, nbMaxs;
      ...
      while(1) {
        ...
        // compute distances
        int d;
        S2_Memset(distances, 0, samplecount*sizeof(int));
