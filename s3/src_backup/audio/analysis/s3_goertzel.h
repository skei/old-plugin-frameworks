#ifndef s2_goertzel_included
#define s2_goertzel_included
//----------------------------------------------------------------------

// http://www.musicdsp.org/showArchiveComment.php?ArchiveID=107

float S2_Goertzel(float* ABuffer, uint32 ALength, float AFrequency, float ASamplerate) {
  float skn = 0;
  float skn1 = 0;
  float skn2 = 0;
  float temp1 = S2_PI2 * AFrequency / ASamplerate;
...
float S2_GoertzelHarmonics(float* ABuffer, uint32 ALength, float AFrequency, float ASamplerate) {
  float qkn = 0;
  float qkn1 = 0;
  float wkn = S2_PI2 * AFrequency / ASamplerate;
  float mk = 2 * cos(wkn);
  for (uint32 i=0; i<ALength; i++) {
    float qkn2 = qkn1;
    qkn1 = qkn;
