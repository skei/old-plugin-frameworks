#ifndef s2_waveform_dsf_included
#define s2_waveform_dsf_included
//----------------------------------------------------------------------

#include <math.h>
#include "common/s2_math_fast.h"
//#include "common/s2_math_table.h"
...
class S2_Waveform_DSF {

  private:

    //S2_MathTable<16384,s2_mt_sin>  MSinTable;
  ...
  public:
    ...
    float processF(float t1, float t2, float a, float N) {
      ...
      float aa          = a * a;
      float a2          = a * 2.0f;
      float w_t1      = S2_Wrap(t1,         -S2_PI,S2_PI);
      float w_t2      = S2_Wrap(t2,         -S2_PI,S2_PI);
      float w_t2t1    = S2_Wrap((t2-t1),    -S2_PI,S2_PI);
      float w_t1nt2   = S2_Wrap((t1+N*t2),  -S2_PI,S2_PI);
      float w_t1n1t2  = S2_Wrap((t1+N1*t2), -S2_PI,S2_PI);
      float sin_t1      = S2_SinF2(w_t1);
      float cos_t2      = S2_CosF2(w_t2);
      float sin_t2t1    = S2_SinF2(w_t2t1);
      float sin_t1nt2   = S2_SinF2(w_t1nt2);
      float sin_t1n1t2  = S2_SinF2(w_t1n1t2);
      //int32 iN1         = (int32)S2_Trunc(N1);
      //float pow_an1     = S2_PowF(a,iN1);
      float pow_an1     = pow(a,N1);
