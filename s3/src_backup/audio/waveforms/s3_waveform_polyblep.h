#ifndef s2_waveform_polyblep
#define s2_waveform_polyblep
//----------------------------------------------------------------------
...
float S2_PolyBlep(float t, float dt) {
  // 0 <= t < 1
  if (t < dt) {
    t /= dt;
...
float S2_PolyBlep2(float t, float dt) {
  if (t<dt) return - S2_Sqr(t/dt - 1.0f);
  else if (t > 1.0f - dt) return S2_Sqr((t - 1.0f) / dt + 1.0f);
  return 0;
}
...
float S2_PolyBlep3(float t, float dt, float h) {
  float t0,c0;
...
/*
float SPolyBlep4(float t, float dt) {
...
/*
float SPolyBlep5(float t, float dt) {
...
/*float SPolyBlep6(float t, float dt) {
...
class S2_Waveform_Saw_PolyBlep {
  public:
    float process(float t, float dt) {
      ...
      if (t >= 1) t -= 1;
      float saw = 2.0f * t - 1;
      saw -= S2_PolyBlep(t,dt);
      return saw;
    }
};

class S2_Waveform_Saw_PolyBlep2 {
  public:
    float process(float t, float dt) {
      // Correct phase, so it would be in line with sin(2.*M_PI * t)
      t += 0.5f;
      if (t >= 1) t -= 1;
      float saw = 2.0f * t - 1;
      saw -= S2_PolyBlep2(t,dt);
      return saw;
    }
};

class S2_Waveform_Saw_PolyBlep3 {
  public:
    float process(float t, float dt) {
      // Correct phase, so it would be in line with sin(2.*M_PI * t)
      t += 0.5f;
...
class S2_Waveform_Squ_PolyBlep {
  public:
...
class S2_Waveform_Tri_PolyBlep {
  private:
    S2_Waveform_Squ_PolyBlep squ;
    float x1;
  public:
    S2_Waveform_Tri_PolyBlep() {
      x1 = 0;
    }
    float process(float t, float dt) {
...
class S2_Waveform_Ramp_PolyBlep {
  public:
    float process(float t, float dt) {
      //t += 0.5f;
      if (t >= 1) t -= 1;
      float ramp = 1 - 2*t;
      ramp += S2_PolyBlep(t,dt);
      return ramp;
    }
};
