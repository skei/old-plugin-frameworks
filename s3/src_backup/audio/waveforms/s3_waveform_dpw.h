#ifndef s2_waveform_dpw
#define s2_waveform_dpw
//----------------------------------------------------------------------
...
class S2_Waveform_Saw_DPW2 {
  private:
    float x1;
  public:
    S2_Waveform_Saw_DPW2() {
      x1 = 0;
    }
    void reset(void) { x1 = 0; }
    float process(float t, float dt) {
      // Correct phase, so it would be in line with sin(2.*S2_PI * t)
      t += (0.5f + (0.5f*dt));
      if (t >= 1) t -= 1;
...
class S2_Waveform_Saw_DPW3 {
  private:
    float x1,x2;
  public:
    S2_Waveform_Saw_DPW3() {
      x1 = 0;
      x2 = 0;
    }
