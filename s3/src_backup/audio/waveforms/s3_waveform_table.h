#ifndef s2_waveform_table_included
#define s2_waveform_table_included
//----------------------------------------------------------------------
...
class S2_Waveform_Table {

  private:

    int32     MNumTables;
    s2_wtable MTables[MAX_TABLES];
  ...
  public:
    ...
    S2_Waveform_Table() {
      MNumTables = 0;
      for (int32 i=0; i<MAX_TABLES; i++) {
        MTables[i].topFreq = 0;
        MTables[i].len = 0;
        MTables[i].table = S2_NULL;//0;
      }
    }
  ...
  public:
    ...
    int32 numTables(void) { return MNumTables; }
  ...
  private:
    ...
    */
    ...
    void fft(int32 N, float *ar, float *ai) {
      /*
      ...
      */
      int32 i, j, k, L;             // indexes
      int32 M, TEMP, LE, LE1, ip;   // M = log N
      int32 NV2, NM1;
      float t;                    // temp
  ...
  public:
    ...
    */
    ...
    int32 addTable(int32 len, float *tableIn, float topFreq) {
      if (MNumTables < MAX_TABLES) {
        float* T = MTables[MNumTables].table = new float[len];
    ...
    */
    ...
    float makeTable(int32 len, float *ar, float *ai, float scale, float topFreq) {
      fft(len, ar, ai);
      if (scale == 0.0) {
        // calc normal
        float max = 0;
      ...
      for (int32 idx = 0; idx < len; idx++) wave[idx] = ai[idx] * scale;
    ...
    */
    ...
    void fillTables(float *freqRe, float *freqIm, int32 numSamples) {
      int32 idx;
      ...
      int32 maxHarmonic = numSamples >> 1;
  ...
  public:
    ...
    //WaveTableOsc*
    void makeSaw(int32 tableLen=2048) {
      //int32 tableLen = 2048;    // to give full bandwidth from 20 Hz
      int32 idx;
      float *freqRe = new float [tableLen];
      float *freqIm = new float [tableLen];
      // make a sawtooth
    ...
    void makeWave(float *waveSamples, int32 tableLen) {
      float *freqRe = new float [tableLen];
      float *freqIm = new float [tableLen];
      // take FFT
      for (int32 idx = 0; idx < tableLen; idx++) {
  ...
  public:
    ...
    */
    ...
    float process(float t, float dt=0) {
      ...
      s2_wtable* wavetable = &MTables[tableIdx];
        ...
        return wavetable->table[ int32(t * wavetable->len) ];
