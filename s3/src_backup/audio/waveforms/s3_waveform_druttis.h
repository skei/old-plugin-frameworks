#ifndef s2_waveform_druttis
#define s2_waveform_druttis
//----------------------------------------------------------------------
...
//class S2_Waveform_Sin_Druttis {
class S2_Waveform_Saw_Druttis {
  ...
  public:
    ...
    float process(float t, float dt) {
      float f = 0.5f;//dt;
      return sin( (((t*f)+_ps)*S2_PI2) + (S2_PI05*_fm*afact(f)) );
    }
...
void S2_Waveform_Druttis_setSampleRate(float sr) {
  _sr = sr;
}
...
float S2_Waveform_Sine_Druttis(float t, float f, float ps, float fm) {
  return sin( (((t*f)+ps)*S2_PI2) + (S2_PI05*fm*_afact(f)) );
}

//----------

float S2_Waveform_Saw_Druttis(float t, float f, float ps, float fm, float fb) {
  float result = S2_Waveform_Sine_Druttis(t,f,ps,fb*S2_Waveform_Sine_Druttis(_prev_t/*t-1*/,f,ps,fm));
  _prev_t = t;
  return result;
}
...
float S2_Waveform_Pulse_Druttis(float t, float f, float ps, float fm, float fb, float pm, float pw) {
  return S2_Waveform_Saw_Druttis(t,f,ps,fm,fb) - S2_Waveform_Saw_Druttis(t,f,ps+(0.5*pw),fm,fb) * pm;
}
