#ifndef s2_filter_linkwitzriley_included
#define s2_filter_linkwitzriley_included
//----------------------------------------------------------------------
...
class S2_Filter_LinkwitzRiley {
  ...
  public:
    ...
    void calcCoeffs(int32 AMode=0) {
      float wc      = S2_PI2 * fc;
      float wc2     = wc*wc;
      float wc3     = wc2*wc;
      float wc4     = wc2*wc2;
      float k       = wc / tan(S2_PI*fc/srate);
      float k2      = k*k;
      float k3      = k2*k;
      float k4      = k2*k2;
      float sqrt2   = sqrt(2);
