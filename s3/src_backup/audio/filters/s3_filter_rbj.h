#ifndef s2_filter_rbj_included
#define s2_filter_rbj_included
//----------------------------------------------------------------------
...
class S2_Filter_RBJ {
  ...
  public:
    ...
    void calcFilterCoeffs(float ASampleRate, uint32 AFilterType, float AFreq, float AQ, float ADBGain, bool AQIsBandWidth) {
    ...
    void CalcFilterCoeffs(float ASampleRate) {
      ...
      if (MFilterType>=6) {
        ...
        omega = S2_PI2*MFreq/ASampleRate;
        tsin = sin(omega);
        tcos = cos(omega);
  ...
  public:
    ...
    uint32  filterType(void)    { return MFilterType; }
    ...
    void    filterType(uint32 AType)          { MFilterType = AType; }
